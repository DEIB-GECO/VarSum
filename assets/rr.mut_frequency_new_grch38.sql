CREATE OR REPLACE FUNCTION rr.mut_frequency_new_grch38(occurrence bigint, males integer, females integer, chrom integer, start_ bigint)
RETURNS numeric AS $$
...
begin
	if occurrence > 0 then
		...
		if chrom<23 then
			...
			if start_ < 2781479 or start_ > 155701383 then
				total_alleles := (males+females)*2;
			else
				total_alleles := males + females*2 ;
			end if;
